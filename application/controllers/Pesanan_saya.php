...
class Pesanan_saya extends CI_Controller
{
	...
	public function bayar($id_order)
	{
		...
		$this->form_validation->set_rules('depe', 'DP', 'required', array('required' => '%s Mohon Untuk Diisi !!!'));
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if (!$this->upload->do_upload($field_name)) {
				$data = array(
					'title' => 'Pembayaran',
					'pesanan' => $this->m_transaksi->detail_pesanan($id_order),
				...
				$depe = $this->input->post('depe');

				$tgl2 = date('Y-m-d', strtotime('+ 7 days', strtotime($tgl)));
				$diskon = 0;
				$diskon = $total - $depe - (5 / 100 * $total);
				if ($tgl <= $tgl2) {
	...
	public function dibatalkan($id_order)
	{
		$data = array(
			'id_order' => $id_order,
			'status_order' => 4
